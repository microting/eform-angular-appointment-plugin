...
if (( "$GIT_STATUS" > 0 )); then
	...
	dotnet add Appointment.Pn.csproj package Microting.eForm
	dotnet add Appointment.Pn.csproj package Microting.eFormApi.BasePn
	dotnet add Appointment.Pn.csproj package Microting.AppointmentBase
	...
	EFORM_PLUGINBASE_VERSION=`dotnet list package | grep 'Microting.AppointmentBase' | cut -c64-71`
	COMMIT_MESSAGE="Updating"$'\n'"- Microting.eForm to ${EFORM_VERSION}"$'\n'"- Microting.eFormApi.BasePn to ${EFORM_BASEPN_VERSION}"$'\n'"- Microting.AppointmentBase to ${EFORM_PLUGINBASE_VERSION}"
